...
if (!TOOL_TESTS) {
    console.log('Skipping tool memory test');
    Deno.exit(0);
}

const code = Deno.readTextFileSync(import.meta.dirname + '/../../tools/memory-insights/tool.ts');
const metadata = JSON.parse(Deno.readTextFileSync(import.meta.dirname + '/../../tools/memory-insights/metadata.json'));
if (!code || !metadata) {
...
Deno.test('Memory test', async (t) => {
    await t.step('Store "Chile" in general memory', async () => {
        ...
        assertEquals(match && match.length > 0, true, 'Output: ' + JSON.stringify(dataA));
    });
    ...
    await t.step('Store "Argentina" in general memory', async () => {
        ...
        assertEquals(match1 && match1.length > 0, true, 'Output: ' + JSON.stringify(dataB));
        const match2 = dataB.general_memory.match(/Argentina/i);
        assertEquals(match2 && match2.length > 0, true, 'Output: ' + JSON.stringify(dataB));
    });
    ...
    await t.step('Store "Spain" in specific memory', async () => {
        ...
        assertEquals(match1 && match1.length > 0, true, 'Output: ' + JSON.stringify(dataC));
        const match2 = dataC.specific_memory.match(/Chile/i);
        assertEquals(!match2 || match2.length === 0, true, 'Output: ' + JSON.stringify(dataC));
        const match3 = dataC.specific_memory.match(/Argentina/i);
        assertEquals(!match3 || match3.length === 0, true, 'Output: ' + JSON.stringify(dataC));
    });
    ...
    await t.step('Store "France" in another specific memory', async () => {
        ...
        assertEquals(match1 && match1.length > 0, true, 'Output: ' + JSON.stringify(dataD));
        const match2 = dataD.specific_memory.match(/Chile/i);
        assertEquals(!match2 || match2.length === 0, true, 'Output: ' + JSON.stringify(dataD));
        const match3 = dataD.specific_memory.match(/Argentina/i);
        assertEquals(!match3 || match3.length === 0, true, 'Output: ' + JSON.stringify(dataD));
    ...
    await t.step('Retrive general memory', async () => {
        ...
        assertEquals(match1 && match1.length > 0, true, 'Output: ' + JSON.stringify(dataE));
        const match2 = dataE.general_memory.match(/Argentina/i);
        assertEquals(match2 && match2.length > 0, true, 'Output: ' + JSON.stringify(dataE));
        const match3 = dataE.general_memory.match(/Spain/i);
        assertEquals(!match3 || match3.length === 0, true, 'Output: ' + JSON.stringify(dataE));
    ...
    await t.step('Retrive specific memory', async () => {
        ...
        assertEquals(match1 && match1.length > 0, true, 'Output: ' + JSON.stringify(dataF));
    });
    ...
    await t.step('Retrive specific memory', async () => {
        ...
        assertEquals(dataG.specific_memory === '' || dataG.specific_memory === null, true, 'Output: ' + JSON.stringify(dataG));
    });
