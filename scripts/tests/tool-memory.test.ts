...
Deno.test('Memory test with custom database name', async (t) => {
    const customDbName = `test-db-${Math.random().toString(36).substring(2, 15)}`;

    // Test 1: Insert "CUSTOM" into "TEST" using custom database
    await t.step('Insert "CUSTOM" into "TEST" with custom database', async () => {
        const result = await runCommandTest({
            action: 'upsert',
            data: 'CUSTOM',
            memory_key: 'TEST'
    ...
    await t.step('Verify "TEST" is not in default database', async () => {
        ...
        assertEquals(result.memory, '', 'Memory should be empty in default database');
    });
