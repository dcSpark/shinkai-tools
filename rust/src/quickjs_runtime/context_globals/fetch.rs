...
use rquickjs::{
    Ctx, IntoJs, Object, Promise, Result
};
use reqwest::{self, header::{HeaderMap, HeaderName, HeaderValue}, Method};

pub fn fetch<'js>(ctx: Ctx<'js>, url: String, options: Object<'js>) -> Result<Promise<'js>> {
    let (promise, resolve, reject) = ctx.promise()?;
    let ctx_clone = ctx.clone();
    ctx.spawn(async move {
        let method = options.get::<String, String>("method".to_string()).unwrap_or_else(|_| "GET".to_string());
        let headers =  options.get::<String, Object>("headers".to_string());
        let body = options.get::<String, Object>("body".to_string());
        ...
        if let Ok(body) = body {
            let body_string = ctx_clone.json_stringify(body).unwrap().unwrap();
            request = request.body::<String>(body_string.to_string().unwrap());
        }
        match request.send().await {
            Ok(response) => {
                ...
                response.headers().iter().for_each(|(key, value)| {
                    ...
                    headers.set(key.as_str(), value.into_js(&ctx_clone)).unwrap();
                });
                match response.text().await {
                    Ok(body_text) => {
                        let body = ctx_clone.json_parse(body_text).unwrap();
                        let response_to_js = Object::new(ctx_clone).unwrap();
