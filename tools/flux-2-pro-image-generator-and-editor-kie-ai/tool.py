    ...
    try:
        ...
        if image_paths:
            ...
            for path in image_paths:
                ...
                # Upload file to get URL with timestamp fix
                original_basename = os.path.basename(path)
                name_part, ext_part = os.path.splitext(original_basename)
                
                # Generate unique filename using timestamp to avoid caching/overwrite issues
                ...
                try:
                    with open(path, 'rb') as f:
                        # Use unique_filename for the upload, keeping local file unchanged
                        files = {'file': (unique_filename, f, mime_type)}
                        data = {'uploadPath': upload_path, 'fileName': unique_filename}
