...
class INPUTS:
    ...
    # input_image_strength: Optional float for strength of the input image in the remix, min 0.01 max 1 step 0.01; defaults to 0.95
    input_image_strength: Optional[float] = 0.95
...
async def run(config: CONFIG, inputs: INPUTS) -> OUTPUT:
    ...
    try:
        ...
        # Validate input_image_strength
        effective_strength = inputs.input_image_strength if inputs.input_image_strength is not None else 0.95
        if not (0.01 <= effective_strength <= 1):
            output.status = "error: input_image_strength must be between 0.01 and 1"
            return output

        # Upload input image to get URL with timestamp
