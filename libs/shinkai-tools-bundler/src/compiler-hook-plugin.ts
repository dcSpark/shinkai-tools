...
type Options = { outputFolder: string, outputFilename: string, outputDefinitionFilename: string };
  ...
  apply(compiler: Compiler) {
    compiler.hooks.assetEmitted.tapAsync(
      ...
      async (assetName, assetInfo, callback) => {
        ...
        const definitionPath = join(this.options.outputFolder, this.options.outputDefinitionFilename);
        writeFileSync(
          definitionPath,
          JSON.stringify(extendedToolDefinition, null, 2),
        );
        console.log(`asset ${this.options.outputDefinitionFilename} emmited from ${assetName}`);
        callback();
      },
    );
  }
